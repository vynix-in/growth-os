...
// HowTo structured data for step-by-step guides. steps is an array of strings.
export function howToLd({ name, description, steps, image }) {
  ...
  return jsonLd({
    ...
    step: steps.map((s, i) => ({ '@type': 'HowToStep', position: i + 1, name: typeof s === 'string' ? s.slice(0, 60) : s.name, text: typeof s === 'string' ? s : s.text })),
  });
}

// ItemList structured data for listicles and listing pages. items is an array
// of { name, url, (optional) description }.
export function itemListLd(items, name) {
  ...
  return jsonLd({
    ...
    itemListElement: items.map((it, i) => ({
      ...
      position: i + 1,
      name: it.name,
      url: it.url ? abs(it.url) : undefined,
    })),
  });
}
...
// DefinedTerm structured data for glossary entries.
export function definedTermLd({ name, description, url }) {
  return jsonLd({
    '@context': 'https://schema.org',
    '@type': 'DefinedTerm',
    name,
    description,
    inDefinedTermSet: { '@type': 'DefinedTermSet', name: 'Vynix Glossary', url: abs('/glossary/') },
    url: abs(url),
  });
}
